...
spec = describe "Analyseur syntaxique d'additions à 1 chiffre" $ do
  ...
  describe "Soustractions" $ do

    it "parse '2-1'" $ do
      exprParser "2-1" `shouldBe` Right (Val 2 `Sub` Val 1)

    it "parse '5-20'" $ do
      exprParser "5-20" `shouldBe` Right (Val 5 `Sub`Val 20)

    it "parse '5-a'" $ do
      exprParser "5-a" `shouldSatisfy` isLeft

  describe "Puissance" $ do
  
    it "parse '5^5'" $ do
      exprParser "5^5" `shouldBe` Right (Val 5 `Pow` Val 5)

    it "parse '16^16'" $ do
       exprParser "16^16" `shouldBe` Right (Val 16 `Pow` Val 16)

  describe "Plusieurs opérations" $ do

     it "parse '5-20+7'" $ do
       exprParser "5-20+7" `shouldBe` Right (Val 5 `Sub` (Val 20 `Plus` Val 7))
